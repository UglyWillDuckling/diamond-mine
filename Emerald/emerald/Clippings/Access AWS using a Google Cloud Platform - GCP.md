...
source: https://aws.amazon.com/blogs/security/access-aws-using-a-google-cloud-platform-native-workload-identity/
about:
  - "[[GCP]]"
category:
  - "[[HowTo]]"
...
The script then returns the **temporary** credentials to the `credential\_process` as the JSON output on STDOUT [^1]; we used [[jq]] to parse the output in the desired JSON format.
