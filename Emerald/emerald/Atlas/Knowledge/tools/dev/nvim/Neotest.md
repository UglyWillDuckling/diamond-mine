...
created: 2025-05-13
published: 
source: https://github.com/nvim-neotest/neotest/tree/862afb2a2219d9ca565f67416fb7003cc0f22c4f
tags:
  - tool/nvim
---
...
[![image](https://user-images.githubusercontent.com/24252670/166156510-440d9047-c76e-4967-8c17-944399222645.png)](https://user-images.githubusercontent.com/24252670/166156510-440d9047-c76e-4967-8c17-944399222645.png)

- [[#Introduction|Introduction]]
- [[#Installation|Installation]]
	- [[#Installation#Supported Runners|Supported Runners]]
- [[#Configuration|Configuration]]
- [[#Usage|Usage]]
- [[#Consumers|Consumers]]
	- [[#Consumers#Watch Tests|Watch Tests]]
	- [[#Consumers#Output Window|Output Window]]
	- [[#Consumers#Output Panel|Output Panel]]
	- [[#Consumers#Summary Window|Summary Window]]
	- [[#Consumers#Diagnostic Messages|Diagnostic Messages]]
	- [[#Consumers#Status Signs|Status Signs]]
- [[#Strategies|Strategies]]
- [[#Writing Adapters|Writing Adapters]]
	- [[#Writing Adapters#Parsing Tests|Parsing Tests]]
	- [[#Writing Adapters#Collecting Results|Collecting Results]]
- [[#Releases 157|Releases 157]]
...
Neotest uses the `CursorHold` event. This uses the `updatetime` setting which is by default very high, and lowering this can lead to excessive writes to disk. It's recommended to use [https://github.com/antoinemadec/FixCursorHold.nvim](https://github.com/antoinemadec/FixCursorHold.nvim) which allows detaching `updatetime` from the frequency of the `CursorHold` event. The repo claims it is no longer needed but it is still recommended (See [this issue](https://github.com/antoinemadec/FixCursorHold.nvim/issues/13))
...
| go | [neotest-go](https://github.com/akinsho/neotest-go)   [neotest-golang](https://github.com/fredrikaverpil/neotest-golang) |
...
For any runner without an adapter you can use [neotest-vim-test](https://github.com/nvim-neotest/neotest-vim-test) which supports any runner that vim-test supports. The vim-test adapter does not support some of the more advanced features such as error locations or per-test output. If you're using the vim-test adapter then install [vim-test](https://github.com/vim-test/vim-test/) too.
...
It is highly recommended to use [lazydev.nvim](https://github.com/folke/lazydev.nvim) to enable type checking for neotest to get type checking, documentation and autocompletion for all API functions.

The default icons use [codicons](https://github.com/microsoft/vscode-codicons). It's recommended to use this [fork](https://github.com/ChristianChiarulli/neovim-codicons) which fixes alignment issues for the terminal. If your terminal doesn't support font fallback and you need to have icons included in your font, you can patch it via [Font Patcher](https://github.com/ryanoasis/nerd-fonts#option-8-patch-your-own-font). There is a simple step by step guide [here](https://github.com/mortepau/codicons.nvim#how-to-patch-fonts).
...
watch.mp4<video src="https://private-user-images.githubusercontent.com/24252670/229367494-6775d7f1-a8fb-461b-bbbd-d6124031293e.mp4?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..yXwc0uanR_SVfZH8zB2NoY9G_buNUjVHMAzwU0J1TFU" controls="controls"></video>

### Output Window

`:h neotest.output`

Displays output of tests [![image](https://user-images.githubusercontent.com/24252670/166143146-e7821fe9-c11c-4e21-9cc0-73989b51e8ed.png)](https://user-images.githubusercontent.com/24252670/166143146-e7821fe9-c11c-4e21-9cc0-73989b51e8ed.png)

Displays per-test output [![image](https://user-images.githubusercontent.com/24252670/166143189-0f51b544-3aec-4cfc-93d7-74f3d209aef6.png)](https://user-images.githubusercontent.com/24252670/166143189-0f51b544-3aec-4cfc-93d7-74f3d209aef6.png)

### Output Panel

`:h neotest.output_panel`

Records all output of tests over time in a single window [![image](https://user-images.githubusercontent.com/24252670/201535290-d726c781-a780-4318-b595-a10832b9f191.png)](https://user-images.githubusercontent.com/24252670/201535290-d726c781-a780-4318-b595-a10832b9f191.png)

### Summary Window

`:h neotest.summary`

Displays test suite structure from project root.[![image](https://user-images.githubusercontent.com/24252670/166143333-df8b409f-d6f3-4d3d-a676-5f8a4a4cb8bb.png)](https://user-images.githubusercontent.com/24252670/166143333-df8b409f-d6f3-4d3d-a676-5f8a4a4cb8bb.png)
...
[![image](https://user-images.githubusercontent.com/24252670/166143466-0fdea24c-6f0a-4199-9026-66f89d7d1dbc.png)](https://user-images.githubusercontent.com/24252670/166143466-0fdea24c-6f0a-4199-9026-66f89d7d1dbc.png)
...
[![image](https://user-images.githubusercontent.com/24252670/166143402-b318ef91-c053-4973-b929-5ee97572f2c2.png)](https://user-images.githubusercontent.com/24252670/166143402-b318ef91-c053-4973-b929-5ee97572f2c2.png)
...
Custom strategies can implemented by providing a function which takes a `neotest.RunSpec` and returns an table that fits the `neotest.Process` interface. Plenary's async library can be used to run asynchronously.
...
Adapters must fulfill an interface to run (defined [here](https://github.com/nvim-neotest/neotest/blob/master/lua/neotest/adapters/interface.lua)).

Much of the functionality is built around using a custom tree object that defines the structure of the test suite. There are helpers that adapters can use within their code (all defined under `neotest.lib`)
...
The query can define capture groups for tests and namespaces. Each type must have `<type>.name` and `<type>.definition` capture groups. They can be used multiple times in the query
...
[\+ 156 releases](https://github.com/nvim-neotest/neotest/releases)

## Sponsor this project

[**rcarriga** Rónán Carrigan](https://github.com/rcarriga)

[Sponsor](https://github.com/sponsors/rcarriga)
...
- [Lua 99.8%](https://github.com/nvim-neotest/neotest/search?l=lua)
- Other 0.2%