...
One exception is the LuaJIT `bit` extension, which is always available: when built with PUC Lua, Nvim includes a fallback implementation which provides `require("bit")`. See [lua-bit](https://neovim.io/doc/user/lua-bit.html#lua-bit).
...
[luv-error-handling](https://neovim.io/doc/user/luvref.html#luv-error-handling)
...
[kwargs](https://neovim.io/doc/user/#kwargs)  
...
[E5560](https://neovim.io/doc/user/#E5560) [lua-loop-callbacks](https://neovim.io/doc/user/#lua-loop-callbacks) It is an error to directly invoke `vim.api` functions (except [api-fast](https://neovim.io/doc/user/api.html#api-fast)) in `vim.uv` callbacks. For example, this is an error:
```lua
...
[inotify-limitations](https://neovim.io/doc/user/#inotify-limitations)  
...
vim.mpack.encode(`{obj}`) [vim.mpack.encode()](https://neovim.io/doc/user/#vim.mpack.encode\(\))  
...
[log\_levels](https://neovim.io/doc/user/#log_levels) [vim.log.levels](https://neovim.io/doc/user/#vim.log.levels) Log levels are one of the values defined in `vim.log.levels`:
...
vim.o [vim.o](https://neovim.io/doc/user/#vim.o)  
...
vim.keycode(`{str}`) [vim.keycode()](https://neovim.io/doc/user/#vim.keycode\(\))  
Translates keycodes.
...
vim.uri\_to\_bufnr(`{uri}`) [vim.uri\_to\_bufnr()](https://neovim.io/doc/user/#vim.uri_to_bufnr\(\))  
...
vim.uri\_to\_fname(`{uri}`) [vim.uri\_to\_fname()](https://neovim.io/doc/user/#vim.uri_to_fname\(\))  
...
vim.ui.input(`{opts}`, `{on_confirm}`) [vim.ui.input()](https://neovim.io/doc/user/#vim.ui.input\(\))  
...
vim.ui.open(`{path}`, `{opt}`) [vim.ui.open()](https://neovim.io/doc/user/#vim.ui.open\(\))  
...
Can also be invoked with `:Open`. [:Open](https://neovim.io/doc/user/#%3AOpen)  
...
vim.ui.select(`{items}`, `{opts}`, `{on_choice}`) [vim.ui.select()](https://neovim.io/doc/user/#vim.ui.select\(\))  
...
"cmd": The cmdline window; also used for ['showcmd'](https://neovim.io/doc/user/options.html#'showcmd'), ['showmode'](https://neovim.io/doc/user/options.html#'showmode'), ['ruler'](https://neovim.io/doc/user/options.html#'ruler'), and messages if ['cmdheight'](https://neovim.io/doc/user/options.html#'cmdheight') > 0.

"box": The message box window; used for messages when ['cmdheight'](https://neovim.io/doc/user/options.html#'cmdheight') == 0.

"more": The more-prompt window; used for [:messages](https://neovim.io/doc/user/message.html#%3Amessages) and certain messages that should be shown in full.

"prompt": The cmdline prompt window; used for prompt messages that expect user input.

These four windows are assigned the "cmdline", "msgbox", "msgmore" and "msgprompt" ['filetype'](https://neovim.io/doc/user/options.html#'filetype') respectively. Use a [FileType](https://neovim.io/doc/user/autocmd.html#FileType) autocommand to configure any local options for these windows and their respective buffers.

## Lua module: vim.filetype

vim.filetype.add(`{filetypes}`) [vim.filetype.add()](https://neovim.io/doc/user/#vim.filetype.add\(\))  
Add new filetype mappings.

Filetype mappings can be added either by extension or by filename (either the "tail" or the full file path). The full file path is checked first, followed by the file name. If a match is not found using the filename, then the filename is matched against the list of [lua-pattern](https://neovim.io/doc/user/luaref.html#lua-pattern) s (sorted by priority) until a match is found. Lastly, if pattern matching does not find a filetype, then the file extension is used.
...
[vim.filetype.get\_option()](https://neovim.io/doc/user/#vim.filetype.get_option\(\))  
vim.filetype.get\_option(`{filetype}`, `{option}`) Get the default option value for a `{filetype}`.
...
**Note:** this uses [nvim\_get\_option\_value()](https://neovim.io/doc/user/api.html#nvim_get_option_value\(\)) but caches the result. This means [ftplugin](https://neovim.io/doc/user/usr_41.html#ftplugin) and [FileType](https://neovim.io/doc/user/autocmd.html#FileType) autocommands are only triggered once and may not reflect later changes.
...
vim.filetype.match(`{args}`) [vim.filetype.match()](https://neovim.io/doc/user/#vim.filetype.match\(\))  
...
vim.keymap.del(`{modes}`, `{lhs}`, `{opts}`) [vim.keymap.del()](https://neovim.io/doc/user/#vim.keymap.del\(\))  
...
vim.keymap.set(`{mode}`, `{lhs}`, `{rhs}`, `{opts}`) [vim.keymap.set()](https://neovim.io/doc/user/#vim.keymap.set\(\))  
Defines a [mapping](https://neovim.io/doc/user/map.html#mapping) of [keycodes](https://neovim.io/doc/user/intro.html#keycodes) to a function or keycodes.
...
`{mode}` (`string|string[]`) Mode "short-name" (see [nvim\_set\_keymap()](https://neovim.io/doc/user/api.html#nvim_set_keymap\(\))), or a list thereof.
...
`{opts}` (`table?`) Table of [:map-arguments](https://neovim.io/doc/user/map.html#%3Amap-arguments). Same as [nvim\_set\_keymap()](https://neovim.io/doc/user/api.html#nvim_set_keymap\(\)) `{opts}`, except:
...
[nvim\_set\_keymap()](https://neovim.io/doc/user/api.html#nvim_set_keymap\(\))
...
[vim.fs.exists()](https://neovim.io/doc/user/#vim.fs.exists\(\))  
Use [uv.fs\_stat()](https://neovim.io/doc/user/luvref.html#uv.fs_stat\(\)) to check a file's type, and whether it exists.
...
vim.fs.abspath(`{path}`) [vim.fs.abspath()](https://neovim.io/doc/user/#vim.fs.abspath\(\))  
...
vim.fs.basename(`{file}`) [vim.fs.basename()](https://neovim.io/doc/user/#vim.fs.basename\(\))  
...
vim.fs.dir(`{path}`, `{opts}`) [vim.fs.dir()](https://neovim.io/doc/user/#vim.fs.dir\(\))  
...
vim.fs.dirname(`{file}`) [vim.fs.dirname()](https://neovim.io/doc/user/#vim.fs.dirname\(\))  
...
vim.fs.find(`{names}`, `{opts}`) [vim.fs.find()](https://neovim.io/doc/user/#vim.fs.find\(\))  
...
vim.fs.joinpath(`{...}`) [vim.fs.joinpath()](https://neovim.io/doc/user/#vim.fs.joinpath\(\))  
...
vim.fs.normalize(`{path}`, `{opts}`) [vim.fs.normalize()](https://neovim.io/doc/user/#vim.fs.normalize\(\))  
...
vim.fs.parents(`{start}`) [vim.fs.parents()](https://neovim.io/doc/user/#vim.fs.parents\(\))  
...
vim.fs.relpath(`{base}`, `{target}`, `{opts}`) [vim.fs.relpath()](https://neovim.io/doc/user/#vim.fs.relpath\(\))  
...
vim.fs.rm(`{path}`, `{opts}`) [vim.fs.rm()](https://neovim.io/doc/user/#vim.fs.rm\(\))  
...
vim.fs.root(`{source}`, `{marker}`) [vim.fs.root()](https://neovim.io/doc/user/#vim.fs.root\(\))  
...
vim.glob.to\_lpeg(`{pattern}`) [vim.glob.to\_lpeg()](https://neovim.io/doc/user/#vim.glob.to_lpeg\(\))  
Parses a raw glob into an [lua-lpeg](https://neovim.io/doc/user/lua.html#lua-lpeg) pattern.

This uses glob semantics from LSP 3.17.0: [https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#pattern](https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#pattern)
...
LPeg is a pattern-matching library for Lua, based on Parsing Expression Grammars (PEGs). [https://bford.info/packrat/](https://bford.info/packrat/)

[lua-lpeg](https://neovim.io/doc/user/#lua-lpeg) [vim.lpeg.Pattern](https://neovim.io/doc/user/#vim.lpeg.Pattern) The LPeg library for parsing expression grammars is included as `vim.lpeg` ([https://www.inf.puc-rio.br/~roberto/lpeg/](https://www.inf.puc-rio.br/~roberto/lpeg/)).

In addition, its regex-like interface is available as [vim.re](https://neovim.io/doc/user/lua.html#vim.re) ([https://www.inf.puc-rio.br/~roberto/lpeg/re.html](https://www.inf.puc-rio.br/~roberto/lpeg/re.html)).

Pattern:match(`{subject}`, `{init}`, `{...}`) [Pattern:match()](https://neovim.io/doc/user/#Pattern%3Amatch\(\))  
...
vim.lpeg.B(`{pattern}`) [vim.lpeg.B()](https://neovim.io/doc/user/#vim.lpeg.B\(\))  
...
vim.lpeg.C(`{patt}`) [vim.lpeg.C()](https://neovim.io/doc/user/#vim.lpeg.C\(\))  
...
vim.lpeg.Carg(`{n}`) [vim.lpeg.Carg()](https://neovim.io/doc/user/#vim.lpeg.Carg\(\))  
...
vim.lpeg.Cb(`{name}`) [vim.lpeg.Cb()](https://neovim.io/doc/user/#vim.lpeg.Cb\(\))  
...
vim.lpeg.Cc(`{...}`) [vim.lpeg.Cc()](https://neovim.io/doc/user/#vim.lpeg.Cc\(\))  
...
vim.lpeg.Cf(`{patt}`, `{func}`) [vim.lpeg.Cf()](https://neovim.io/doc/user/#vim.lpeg.Cf\(\))  
...
vim.lpeg.Cg(`{patt}`, `{name}`) [vim.lpeg.Cg()](https://neovim.io/doc/user/#vim.lpeg.Cg\(\))  
...
vim.lpeg.Cmt(`{patt}`, `{fn}`) [vim.lpeg.Cmt()](https://neovim.io/doc/user/#vim.lpeg.Cmt\(\))  
...
vim.lpeg.Cp() [vim.lpeg.Cp()](https://neovim.io/doc/user/#vim.lpeg.Cp\(\))  
...
vim.lpeg.Cs(`{patt}`) [vim.lpeg.Cs()](https://neovim.io/doc/user/#vim.lpeg.Cs\(\))  
...
vim.lpeg.Ct(`{patt}`) [vim.lpeg.Ct()](https://neovim.io/doc/user/#vim.lpeg.Ct\(\))  
...
vim.lpeg.locale(`{tab}`) [vim.lpeg.locale()](https://neovim.io/doc/user/#vim.lpeg.locale\(\))  
...
vim.lpeg.match(`{pattern}`, `{subject}`, `{init}`, `{...}`) [vim.lpeg.match()](https://neovim.io/doc/user/#vim.lpeg.match\(\))  
Matches the given `pattern` against the `subject` string. If the match succeeds, returns the index in the subject of the first character after the match, or the captured values (if the pattern captured any value). An optional numeric argument `init` makes the match start at that position in the subject string. As usual in Lua libraries, a negative value counts from the end. Unlike typical pattern-matching functions, `match` works only in anchored mode; that is, it tries to match the pattern with a prefix of the given subject string (at position `init`), not with an arbitrary substring of the subject. So, if we want to find a pattern anywhere in a string, we must either write a loop in Lua or write a pattern that matches anywhere.
...
vim.lpeg.P(`{value}`) [vim.lpeg.P()](https://neovim.io/doc/user/#vim.lpeg.P\(\))  
...
vim.lpeg.R(`{...}`) [vim.lpeg.R()](https://neovim.io/doc/user/#vim.lpeg.R\(\))  
...
vim.lpeg.S(`{string}`) [vim.lpeg.S()](https://neovim.io/doc/user/#vim.lpeg.S\(\))  
...
vim.lpeg.setmaxstack(`{max}`) [vim.lpeg.setmaxstack()](https://neovim.io/doc/user/#vim.lpeg.setmaxstack\(\))  
...
vim.lpeg.type(`{value}`) [vim.lpeg.type()](https://neovim.io/doc/user/#vim.lpeg.type\(\))  
...
vim.lpeg.V(`{v}`) [vim.lpeg.V()](https://neovim.io/doc/user/#vim.lpeg.V\(\))  
...
vim.lpeg.version() [vim.lpeg.version()](https://neovim.io/doc/user/#vim.lpeg.version\(\))  
...
See [https://www.inf.puc-rio.br/~roberto/lpeg/re.html](https://www.inf.puc-rio.br/~roberto/lpeg/re.html) for the original documentation including regex syntax and examples.

vim.re.compile(`{string}`, `{defs}`) [vim.re.compile()](https://neovim.io/doc/user/#vim.re.compile\(\))  
...
vim.re.find(`{subject}`, `{pattern}`, `{init}`) [vim.re.find()](https://neovim.io/doc/user/#vim.re.find\(\))  
...
vim.re.gsub(`{subject}`, `{pattern}`, `{replacement}`) [vim.re.gsub()](https://neovim.io/doc/user/#vim.re.gsub\(\))  
...
vim.re.match(`{subject}`, `{pattern}`, `{init}`) [vim.re.match()](https://neovim.io/doc/user/#vim.re.match\(\))  
...
vim.re.updatelocale() [vim.re.updatelocale()](https://neovim.io/doc/user/#vim.re.updatelocale\(\))  
...
[regex:match\_line()](https://neovim.io/doc/user/#regex%3Amatch_line\(\))  
regex:match\_line(`{bufnr}`, `{line_idx}`, `{start}`, `{end_}`) Matches line at `line_idx` (zero-based) in buffer `bufnr`. Match is restricted to byte index range `start` and `end_` if given, otherwise see [regex:match\_str()](https://neovim.io/doc/user/lua.html#regex%3Amatch_str\(\)). Returned byte indices are relative to `start` if given.
...
regex:match\_str(`{str}`) [regex:match\_str()](https://neovim.io/doc/user/#regex%3Amatch_str\(\))  
...
vim.regex(`{re}`) [vim.regex()](https://neovim.io/doc/user/#vim.regex\(\))  
Parses the Vim regex `re` and returns a regex object. Regexes are "magic" and case-sensitive by default, regardless of ['magic'](https://neovim.io/doc/user/options.html#'magic') and ['ignorecase'](https://neovim.io/doc/user/options.html#'ignorecase'). They can be controlled with flags, see [/magic](https://neovim.io/doc/user/pattern.html#%2Fmagic) and [/ignorecase](https://neovim.io/doc/user/pattern.html#%2Fignorecase).
...
vim.secure.read(`{path}`) [vim.secure.read()](https://neovim.io/doc/user/#vim.secure.read\(\))  
...
The user's choice is persisted in a trust database at $XDG\_STATE\_HOME/nvim/trust.
...
vim.secure.trust(`{opts}`) [vim.secure.trust()](https://neovim.io/doc/user/#vim.secure.trust\(\))  
Manage the trust database.

The trust database is located at [$XDG\_STATE\_HOME](https://neovim.io/doc/user/starting.html#%24XDG_STATE_HOME) /nvim/trust.
...
[vim.version()](https://neovim.io/doc/user/#vim.version\(\)) returns the version of the current Nvim process.
...
vim.version.cmp(`{v1}`, `{v2}`) [vim.version.cmp()](https://neovim.io/doc/user/#vim.version.cmp\(\))  
...
vim.version.eq(`{v1}`, `{v2}`) [vim.version.eq()](https://neovim.io/doc/user/#vim.version.eq\(\))  
...
vim.version.ge(`{v1}`, `{v2}`) [vim.version.ge()](https://neovim.io/doc/user/#vim.version.ge\(\))  
...
vim.version.gt(`{v1}`, `{v2}`) [vim.version.gt()](https://neovim.io/doc/user/#vim.version.gt\(\))  
Returns `true` if `v1 > v2`. See [vim.version.cmp()](https://neovim.io/doc/user/lua.html#vim.version.cmp\(\)) for usage.
...
vim.version.last(`{versions}`) [vim.version.last()](https://neovim.io/doc/user/#vim.version.last\(\))  
...
vim.version.le(`{v1}`, `{v2}`) [vim.version.le()](https://neovim.io/doc/user/#vim.version.le\(\))  
Returns `true` if `v1 <= v2`. See [vim.version.cmp()](https://neovim.io/doc/user/lua.html#vim.version.cmp\(\)) for usage.
...
vim.version.lt(`{v1}`, `{v2}`) [vim.version.lt()](https://neovim.io/doc/user/#vim.version.lt\(\))  
Returns `true` if `v1 < v2`. See [vim.version.cmp()](https://neovim.io/doc/user/lua.html#vim.version.cmp\(\)) for usage.
...
vim.version.parse(`{version}`, `{opts}`) [vim.version.parse()](https://neovim.io/doc/user/#vim.version.parse\(\))  
...
vim.version.range(`{spec}`) [vim.version.range()](https://neovim.io/doc/user/#vim.version.range\(\))  
...
Tables with a [\_\_call()](https://neovim.io/doc/user/luaref.html#__call\(\)) metamethod are treated as function iterators.
...
Iter:all(`{pred}`) [Iter:all()](https://neovim.io/doc/user/#Iter%3Aall\(\))  
...
Iter:any(`{pred}`) [Iter:any()](https://neovim.io/doc/user/#Iter%3Aany\(\))  
...
Iter:each(`{f}`) [Iter:each()](https://neovim.io/doc/user/#Iter%3Aeach\(\))  
...
Iter:enumerate() [Iter:enumerate()](https://neovim.io/doc/user/#Iter%3Aenumerate\(\))  
...
Iter:filter(`{f}`) [Iter:filter()](https://neovim.io/doc/user/#Iter%3Afilter\(\))  
...
Iter:find(`{f}`) [Iter:find()](https://neovim.io/doc/user/#Iter%3Afind\(\))  
...
Iter:flatten(`{depth}`) [Iter:flatten()](https://neovim.io/doc/user/#Iter%3Aflatten\(\))  
...
Iter:fold(`{init}`, `{f}`) [Iter:fold()](https://neovim.io/doc/user/#Iter%3Afold\(\))  
Folds ("reduces") an iterator into a single value. [Iter:reduce()](https://neovim.io/doc/user/#Iter%3Areduce\(\))  
...
Iter:join(`{delim}`) [Iter:join()](https://neovim.io/doc/user/#Iter%3Ajoin\(\))  
...
Iter:last() [Iter:last()](https://neovim.io/doc/user/#Iter%3Alast\(\))  
...
Iter:map(`{f}`) [Iter:map()](https://neovim.io/doc/user/#Iter%3Amap\(\))  
...
Iter:next() [Iter:next()](https://neovim.io/doc/user/#Iter%3Anext\(\))  
...
Iter:nth(`{n}`) [Iter:nth()](https://neovim.io/doc/user/#Iter%3Anth\(\))  
...
Iter:peek() [Iter:peek()](https://neovim.io/doc/user/#Iter%3Apeek\(\))  
...
Iter:pop() [Iter:pop()](https://neovim.io/doc/user/#Iter%3Apop\(\))  
...
Iter:rev() [Iter:rev()](https://neovim.io/doc/user/#Iter%3Arev\(\))  
...
Iter:rfind(`{f}`) [Iter:rfind()](https://neovim.io/doc/user/#Iter%3Arfind\(\))  
...
Iter:rpeek() [Iter:rpeek()](https://neovim.io/doc/user/#Iter%3Arpeek\(\))  
...
Iter:rskip(`{n}`) [Iter:rskip()](https://neovim.io/doc/user/#Iter%3Arskip\(\))  
...
Iter:skip(`{n}`) [Iter:skip()](https://neovim.io/doc/user/#Iter%3Askip\(\))  
...
Iter:slice(`{first}`, `{last}`) [Iter:slice()](https://neovim.io/doc/user/#Iter%3Aslice\(\))  
...
Iter:take(`{n}`) [Iter:take()](https://neovim.io/doc/user/#Iter%3Atake\(\))  
...
Iter:totable() [Iter:totable()](https://neovim.io/doc/user/#Iter%3Atotable\(\))  
...
[vim.snippet.ActiveFilter](https://neovim.io/doc/user/#vim.snippet.ActiveFilter)

vim.snippet.active(`{filter}`) [vim.snippet.active()](https://neovim.io/doc/user/#vim.snippet.active\(\))  
...
vim.snippet.expand(`{input}`) [vim.snippet.expand()](https://neovim.io/doc/user/#vim.snippet.expand\(\))  
Expands the given snippet text. Refer to [https://microsoft.github.io/language-server-protocol/specification/#snippet\_syntax](https://microsoft.github.io/language-server-protocol/specification/#snippet_syntax) for the specification of valid input.
...
vim.snippet.jump(`{direction}`) [vim.snippet.jump()](https://neovim.io/doc/user/#vim.snippet.jump\(\))  
...
vim.snippet.stop() [vim.snippet.stop()](https://neovim.io/doc/user/#vim.snippet.stop\(\))  
Exits the current snippet.

## Lua module: vim.text

vim.text.hexdecode(`{enc}`) [vim.text.hexdecode()](https://neovim.io/doc/user/#vim.text.hexdecode\(\))  
Hex decode a string.
...
vim.text.hexencode(`{str}`) [vim.text.hexencode()](https://neovim.io/doc/user/#vim.text.hexencode\(\))  
Hex encode a string.
...
vim.text.indent(`{size}`, `{text}`, `{opts}`) [vim.text.indent()](https://neovim.io/doc/user/#vim.text.indent\(\))  
...
:\[range\]TOhtml `{file}`[:TOhtml](https://neovim.io/doc/user/#%3ATOhtml)  
...
tohtml.tohtml(`{winid}`, `{opt}`) [tohtml.tohtml.tohtml()](https://neovim.io/doc/user/#tohtml.tohtml.tohtml\(\))  
