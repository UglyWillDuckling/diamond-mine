...
-   *feature_search* Features
    ...
    -   [🧠 Embeddings](index.html){.sidebar-nested-link}
...
-   [🧠 Embeddings]{itemprop="name"}

On this page

-   -   [Model compatibility](index.html#model-compatibility)
    -   [Manual Setup](index.html#manual-setup)
    -   [Huggingface embeddings](index.html#huggingface-embeddings)
    -   [Llama.cpp embeddings](index.html#llamacpp-embeddings)
...
Create a `YAML` config file in the `models` directory. Specify the
`backend` and the model file.

``` {#6624948 .language-yaml}
...
To use `sentence-transformers` and models in `huggingface` you can use
the `sentencetransformers` embedding backend.

``` {#744d18b .language-yaml}
...
The `sentencetransformers` backend uses Python
[sentence-transformers![](data:image/svg+xml;base64,)](https://github.com/UKPLab/sentence-transformers){rel="external"
target="_blank"}. For a list of all pre-trained models available see
here:
[https://github.com/UKPLab/sentence-transformers#pre-trained-models![](data:image/svg+xml;base64,)](https://github.com/UKPLab/sentence-transformers#pre-trained-models){rel="external"
target="_blank"}

[notifications]{.material-icons .size-20 .me-2}

-   The `sentencetransformers` backend is an optional backend of LocalAI
    and uses Python. If you are running `LocalAI` from the containers
    you are good to go and should be already configured for use.

-   If you are running `LocalAI` manually you must install the python
    dependencies (`make prepare-extra-conda-environments`). This
    requires `conda` to be installed.

-   For local execution, you also have to specify the extra backend in
    the `EXTERNAL_GRPC_BACKENDS` environment variable.

    -   Example:
        `EXTERNAL_GRPC_BACKENDS="sentencetransformers:/path/to/LocalAI/backend/python/sentencetransformers/sentencetransformers.py"`

-   The `sentencetransformers` backend does support only embeddings of
    text, and not of tokens. If you need to embed tokens you can use the
    `bert` backend or `llama.cpp`.

-   No models are required to be downloaded before using the
    `sentencetransformers` backend. The models will be downloaded
    automatically the first time the API is used.

## Llama.cpp embeddings [*link*](index.html#llamacpp-embeddings){.anchor aria-hidden="true"} {#llamacpp-embeddings}

Embeddings with `llama.cpp` are supported with the `llama-cpp` backend,
it needs to be enabled with `embeddings` set to `true`.

``` {#473d489 .language-yaml}
...
-   Example that uses LLamaIndex and LocalAI as embedding:
...
page](https://github.com/mudler/LocalAI/blob/master/docs/content/docs/features/embeddings.md){alt="🧠 Embeddings"
rel="noopener noreferrer" target="_blank"}

Last updated [27 Nov 2024, 16:34 +0100 ]{#relativetime
authdate="2024-11-27T16:34:28+0100" title="27 Nov 2024, 16:34 +0100"}.
...
*navigate_before* 🎨 Image generation

[](../gpt-vision/index.html){.ms-auto}

🥽 GPT Vision *navigate_next*
